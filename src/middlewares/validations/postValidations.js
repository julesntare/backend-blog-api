...
const postAddValidations = async (req, res, next) => {
	...
	if (req.body.author == undefined || req.body.title == undefined) {
		return res.status(403).json({ msg: 'Please provide at least author and title' });
	}
	if (!/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/.test(req.body.author)) {
		return res.status(403).json({ msg: 'author must not be empty, must be email, and have above 3 letters' });
	}
	if (req.body.title.length < 3 || req.body.title == '' || typeof req.body.title != 'string') {
		return res.status(403).json({ msg: 'title must not be empty, be string and have above 3 letters' });
	}
	if (userData == null) {
		return res.status(403).json({ msg: 'Author not available in system' });
	}
	if (req.body.desc) {
		if (req.body.desc == '' || typeof req.body.desc != 'string') {
			return res.status(403).json({ msg: 'desc must not be empty' });
		}
	}
	if (req.body.state) {
		if (req.body.state != 'published' || req.body.state != 'draft' || typeof req.body.state != 'string') {
			return res.status(403).json({ msg: 'state must be string and have to be "published" or "draft"' });
		}
	}
	next();
};
